...
# Help weasyprint find Pango/Cairo installed via Homebrew on macOS.
# weasyprint loads native libs via ctypes at import time; macOS won't
# search /opt/homebrew/lib by default, so we hint dyld explicitly.
# Without this, PDF generation either fails or falls back to a serif font
# because system-ui / sans-serif can't be resolved. Harmless on Linux
# (Railway's Dockerfile already has the libs in standard paths).
# Mirrors the same preamble in streamlit_app_v8.py.
...
def convert_html_to_pdf(html_content: str) -> bytes:
    ...
    # 3. Navigation bar is hidden via print CSS (.nav-bar { display:none }) below.
    #    We do NOT strip it with a regex: the old non-greedy pattern ran past the
    #    nav-bar (which never contains 3 consecutive </div>) and ate following
    #    report sections, leaving unbalanced markup that crashed weasyprint with a
    #    bare AssertionError on some inputs. display:none keeps it out of the PDF
    #    without corrupting the document tree.
    ...
    # 5b. Escape stray "<" that is NOT the start of a real tag (tags begin with a
    #     letter, "/", or "!"). AI-written values legitimately contain "<", e.g. the
    #     benchmark string "<= 4.0x" or narrative like "DSCR < 1.0x". Injected raw,
    #     these confuse weasyprint's parser. JavaScript (which uses "<" in comparisons)
    #     has already been removed above, and CSS does not contain "<", so this only
    #     touches data text. Done last so it never alters the tags we matched earlier.
    pdf_html = re.sub(r'<(?![/!a-zA-Z])', '&lt;', pdf_html)
...
body {
    ...
    padding: 0 !important;
    margin: 0 !important;
    font-family: Inter, system-ui, -apple-system, 'Symbola', sans-serif !important;
}
...
/* Page break controls.
   IMPORTANT: weasyprint raises AssertionError ("assert not page_is_empty")
   when an element with page-break-inside:avoid is TALLER than a single page,
   because it cannot place it even on a fresh page. The base stylesheet's
   @media print sets .collapsible-section{page-break-inside:avoid}, and whole
   report sections (P&L, Balance Sheet, Summary) plus long AI-written narrative
   boxes routinely exceed a page. These MUST stay breakable, so we override the
   base rule here and only keep "avoid" on elements that are reliably short. */
.collapsible-section, .section-content, .audit-opinion-card, .table-card,
.obs-box, .reco-box, .facility-box, .key-obs-item, .audit-opinion-item,
.note-box, .note-box.warning, .note-box.info,
.report-footer, footer { page-break-inside: auto !important; }
...
# =============================================================================
# FINANCIAL CONSISTENCY VALIDATOR SUITE
# =============================================================================
# check_mathematical_integrity above only reads the model's own integrity_check
# self-report — a JSON whose line items don't sum, whose balance sheet is
# missing whole sections, or whose ratios disagree with the statements still
# shows "Balanced". These validators recompute every total, accounting
# identity, ratio, and cross-section figure from the raw line items, so a
# wrong / missing / misclassified number is caught before the report reaches
# an analyst. On the v8-api branch the errors also feed the analyze.py
# self-correction retry loop, so the model fixes its own output.
#
# Error messages are written FOR the model (path + stated value + recomputed
# value + literal arithmetic + fix instruction), matching the DSCR validators.

SUM_TOLERANCE = 2.0     # RM; absorbs cent-rounding across summed line items
RATIO_TOLERANCE = 0.02  # absolute, for "x" ratios stored at 2 dp
PCT_TOLERANCE = 0.06    # percentage points, for % ratios stored at 2 dp
DAYS_TOLERANCE = 1.5    # day-count ratios are stored as integers

def _node_values(node) -> Dict:
    """Period->value dict for a node that carries values directly or nests
    them under its own 'total' (sections and PPE-style items)."""
    ...
    if isinstance(total, dict) and isinstance(total.get("values"), dict):
        return total["values"]
    return {}

def _num(node, pk: str):
    v = _node_values(node).get(pk)
...
def _section_items(section) -> Dict:
    """Effective item dict of a balance-sheet section. Some outputs nest the
    breakdown under a 'line_items' child (sometimes alongside flattened
    duplicates of the same items) — in that case line_items is authoritative
    and the flattened copies are display duplicates that must not be summed
    twice."""
    if not isinstance(section, dict):
        return {}
    line_items = section.get("line_items")
    if isinstance(line_items, dict) and line_items:
        return line_items
    return {k: v for k, v in section.items()
            if k not in ("total", "line_items") and isinstance(v, dict)}

def _get_item(section, key: str):
    """Find an item by key whether the section is flat or line_items-nested."""
    if not isinstance(section, dict):
        return {}
    item = section.get(key)
    if isinstance(item, dict) and _node_values(item):
        return item
    nested = section.get("line_items", {})
...
def check_statement_completeness(data: Dict) -> List[str]:
    """Every balance-sheet section and P&L stage must exist and carry a value
    for every analysed period. The renderer silently drops missing sections
    (a real production run once rendered a balance sheet that stopped at
    TOTAL ASSETS — no equity, no liabilities — and still showed 'Balanced')."""
    issues: List[str] = []
    period_keys = get_period_keys(data)
    if not period_keys:
        return issues
    bs = get_balance_sheet(data)
    pnl = get_income_statement(data)
    if isinstance(bs, dict) and bs:
        ...
        for key in bs_required:
            node = bs.get(key)
            if not isinstance(node, dict) or not node:
                issues.append(
                    f"BALANCE SHEET INCOMPLETE: statement_of_financial_position.{key} "
                    f"is missing or empty. The renderer silently omits missing sections, "
                    f"producing a truncated balance sheet. Emit the full section with its "
                    f"line items and total for every period ({', '.join(period_keys)})."
                )
                continue
            vals = _node_values(node)
            missing = [pk for pk in period_keys if not isinstance(vals.get(pk), (int, float))]
            if missing:
                issues.append(
                    f"BALANCE SHEET INCOMPLETE: statement_of_financial_position.{key} "
                    f"has no numeric total for period(s) {missing}. Every analysed period "
                    f"must carry a value in every balance sheet section (use 0 only when "
                    f"the source statement genuinely reports nil)."
                )
    if isinstance(pnl, dict) and pnl:
        pnl_required = [
            "revenue", "cost_of_sales", "gross_profit", "operating_expenses",
            "operating_profit", "finance_costs", "profit_before_tax",
            "taxation", "net_profit_after_tax", "ebitda",
        ]
        for key in pnl_required:
            node = pnl.get(key)
            if not isinstance(node, dict) or not node:
                issues.append(
                    f"P&L INCOMPLETE: statement_of_comprehensive_income.{key} is missing "
                    f"or empty. Emit it with values for every period ({', '.join(period_keys)})."
                )
    return issues

def _check_container_sum(issues: List[str], path: str, container, label: str) -> None:
    """Containers shaped {line_items: {...}, total: {values: {...}}} — the
    stated total must equal the sum of its own line items, per period."""
    if not isinstance(container, dict):
        return
    line_items = container.get("line_items")
    totals = _node_values(container.get("total", {}))
    if not isinstance(line_items, dict) or not isinstance(totals, dict):
        return
    for pk, stated in totals.items():
        if not isinstance(stated, (int, float)):
            continue
        subtotal, seen = 0.0, False
        for item in line_items.values():
            ...
            if v is not None:
                ...
                seen = True
        if not seen:
            if abs(stated) > SUM_TOLERANCE:
                issues.append(
                    f"{label}: {path}.total.values.{pk} = {_fmt(stated)} but line_items "
                    f"carries no value for this period. Every total must be broken down "
                    f"into the line items that produce it (emit a single line item when "
                    f"the source is unitemised)."
                )
            continue
        diff = stated - subtotal
        if abs(diff) > SUM_TOLERANCE:
            issues.append(
                f"SUM MISMATCH in {label}: {path}.total.values.{pk} = {_fmt(stated)} but "
                f"its line_items sum to {_fmt(subtotal)} (difference {_fmt(diff)}). "
                f"Re-extract this section from the source documents — a line item is "
                f"missing, duplicated, or misclassified, or the stated total is wrong. "
                f"Fix the line items and the total so they agree, then propagate the "
                f"correction to every dependent total and ratio."
            )

def _check_section_sum(issues: List[str], path: str, section, label: str) -> None:
    """Balance-sheet sections — the section total must equal the sum of its
    child items. Children may carry values directly or nest them under their
    own 'total' (e.g. property_plant_equipment)."""
    if not isinstance(section, dict):
        return
    totals = _node_values(section.get("total", {}))
    ...
    for pk, stated in totals.items():
        ...
        if abs(diff) > SUM_TOLERANCE:
            issues.append(
                f"SUM MISMATCH in {label}: {path}.total.values.{pk} = {_fmt(stated)} but "
                f"the section's items sum to {_fmt(subtotal)} (difference {_fmt(diff)}). "
                f"A line item is missing, double-counted, or sitting in the wrong section "
                f"(e.g. a collateral/sinking-fund balance counted inside both cash and "
                f"deposits). Re-extract the section so the items and the total agree, "
                f"then propagate the correction to every dependent total and ratio."
            )

def check_section_sums(data: Dict) -> List[str]:
    issues: List[str] = []
    pnl = get_income_statement(data)
    if isinstance(pnl, dict):
        for key, label in [
            ...
            _check_container_sum(issues, f"statement_of_comprehensive_income.{key}",
                                 pnl.get(key), label)
        opex = pnl.get("operating_expenses")
        if isinstance(opex, dict):
            for sub in ("administrative_expenses", "other_expenses"):
                _check_container_sum(
                    issues, f"statement_of_comprehensive_income.operating_expenses.{sub}",
                    opex.get(sub), f"Operating Expenses ({snake_to_title(sub)})")
    bs = get_balance_sheet(data)
    if isinstance(bs, dict):
        for key, label in [
            ...
            _check_section_sum(issues, f"statement_of_financial_position.{key}",
                               bs.get(key), label)
        ppe = bs.get("non_current_assets", {}).get("property_plant_equipment") \
            if isinstance(bs.get("non_current_assets"), dict) else None
        _check_container_sum(
            issues, "statement_of_financial_position.non_current_assets.property_plant_equipment",
            ppe, "Property, Plant & Equipment")
    return issues

def check_balance_sheet_identities(data: Dict) -> List[str]:
    """Recompute the accounting identities independently of the model's
    integrity_check self-report, and verify the self-report itself restates
    the balance sheet figures honestly."""
    issues: List[str] = []
    bs = get_balance_sheet(data)
    if not isinstance(bs, dict) or not bs:
        return issues
    nca = _node_values(bs.get("non_current_assets", {}))
    ca = _node_values(bs.get("current_assets", {}))
    eq = _node_values(bs.get("equity", {}))
    ncl = _node_values(bs.get("non_current_liabilities", {}))
    cl = _node_values(bs.get("current_liabilities", {}))
    ta = _node_values(bs.get("total_assets", {}))
    tl = _node_values(bs.get("total_liabilities", {}))
    tel = _node_values(bs.get("total_equity_and_liabilities", {}))
    for pk in get_period_keys(data):
        identities = [
            ("total_assets", ta.get(pk), (nca.get(pk), ca.get(pk)),
             "non_current_assets.total + current_assets.total"),
            ("total_liabilities", tl.get(pk), (ncl.get(pk), cl.get(pk)),
             "non_current_liabilities.total + current_liabilities.total"),
            ("total_equity_and_liabilities", tel.get(pk), (eq.get(pk), tl.get(pk)),
             "equity.total + total_liabilities"),
        ]
        for name, stated, parts, formula in identities:
            if not isinstance(stated, (int, float)):
                continue
            if not all(isinstance(p, (int, float)) for p in parts):
                continue
            computed = sum(parts)
            if abs(stated - computed) > SUM_TOLERANCE:
                issues.append(
                    f"IDENTITY MISMATCH for {pk}: statement_of_financial_position.{name} "
                    f"= {_fmt(stated)} but {formula} = {_fmt(parts[0])} + {_fmt(parts[1])} "
                    f"= {_fmt(computed)} (difference {_fmt(stated - computed)}). "
                    f"Recompute and make all three figures agree."
                )
        a, b = ta.get(pk), tel.get(pk)
        if isinstance(a, (int, float)) and isinstance(b, (int, float)) and abs(a - b) > SUM_TOLERANCE:
            issues.append(
                f"BALANCE SHEET DOES NOT BALANCE for {pk}: total_assets = {_fmt(a)} vs "
                f"total_equity_and_liabilities = {_fmt(b)} (variance {_fmt(a - b)}). "
                f"Find the extraction error on whichever side disagrees with the source "
                f"documents and correct it — do NOT plug the difference into an invented "
                f"line item."
            )
    ic = data.get("integrity_check", {})
    bs_check = ic.get("balance_sheet_verification", {}) if isinstance(ic, dict) else {}
    if isinstance(bs_check, dict):
        for pk, blk in bs_check.items():
            if not isinstance(blk, dict):
                continue
            for field, ref in (("total_assets", ta), ("total_equity_and_liabilities", tel)):
                v, r = blk.get(field), ref.get(pk)
                if isinstance(v, (int, float)) and isinstance(r, (int, float)) and abs(v - r) > SUM_TOLERANCE:
                    issues.append(
                        f"INTEGRITY CHECK DRIFT: integrity_check.balance_sheet_verification."
                        f"{pk}.{field} = {_fmt(v)} does not match the balance sheet's "
                        f"{field} = {_fmt(r)}. The integrity check must restate the balance "
                        f"sheet figures exactly — never report different numbers."
                    )
    return issues

def check_pnl_chain(data: Dict) -> List[str]:
    """Recompute the P&L waterfall: GP, EBIT, PBT, NPAT, EBITDA."""
    issues: List[str] = []
    pnl = get_income_statement(data)
    if not isinstance(pnl, dict) or not pnl:
        return issues
    rev = _node_values(pnl.get("revenue", {}))
    cos = _node_values(pnl.get("cost_of_sales", {}))
    gp = _node_values(pnl.get("gross_profit", {}))
    oi = _node_values(pnl.get("other_income", {}))
    op = _node_values(pnl.get("operating_profit", {}))
    fc = _node_values(pnl.get("finance_costs", {}))
    pbt = _node_values(pnl.get("profit_before_tax", {}))
    tax = _node_values(pnl.get("taxation", {}))
    npat = _node_values(pnl.get("net_profit_after_tax", {}))
    ebitda = _node_values(pnl.get("ebitda", {}))
    dep = pnl.get("ebitda", {}).get("depreciation_source", {}) \
        if isinstance(pnl.get("ebitda"), dict) else {}
    opex = pnl.get("operating_expenses", {})
    adm = _node_values(opex.get("administrative_expenses", {})) if isinstance(opex, dict) else {}
    oth = _node_values(opex.get("other_expenses", {})) if isinstance(opex, dict) else {}

    def _chain(pk, name, stated, parts, arithmetic):
        if not isinstance(stated, (int, float)):
            return
        if any(not isinstance(p, (int, float)) for p in parts):
            return
        computed = parts[0]
        for sign, p in zip(arithmetic, parts[1:]):
            computed = computed + p if sign == "+" else computed - p
        if abs(stated - computed) > SUM_TOLERANCE:
            issues.append(
                f"P&L CHAIN MISMATCH for {pk}: statement_of_comprehensive_income.{name} "
                f"= {_fmt(stated)} but its formula recomputes to {_fmt(computed)} "
                f"(difference {_fmt(stated - computed)}). Recompute the waterfall "
                f"(GP = Revenue - COS; EBIT = GP + Other Income - Operating Expenses; "
                f"PBT = EBIT - Finance Costs; NPAT = PBT - Taxation; "
                f"EBITDA = EBIT + Depreciation) and make every stage consistent."
            )

    for pk in get_period_keys(data):
        _chain(pk, "gross_profit", gp.get(pk), [rev.get(pk), cos.get(pk)], "-")
        if isinstance(adm.get(pk), (int, float)) or isinstance(oth.get(pk), (int, float)):
            total_opex = (adm.get(pk) or 0) + (oth.get(pk) or 0)
            _chain(pk, "operating_profit", op.get(pk),
                   [gp.get(pk), (oi.get(pk) if isinstance(oi.get(pk), (int, float)) else 0), total_opex], "+-")
        _chain(pk, "profit_before_tax", pbt.get(pk), [op.get(pk), fc.get(pk)], "-")
        _chain(pk, "net_profit_after_tax", npat.get(pk), [pbt.get(pk), tax.get(pk)], "-")
        d = dep.get(pk) if isinstance(dep, dict) else None
        if isinstance(d, (int, float)):
            _chain(pk, "ebitda", ebitda.get(pk), [op.get(pk), d], "+")
    return issues

def check_equity_articulation(data: Dict) -> List[str]:
    """Tie the income statement to the balance sheet. Between two consecutive
    AUDITED periods the movement in retained earnings must equal net profit
    less dividends. This is the only check that compares the P&L against an
    INDEPENDENT source (the balance sheet), so it catches a P&L that foots
    internally but is still wrong — e.g. an expense or sub-total mis-read that
    flows cleanly down to NPAT yet leaves NPAT out of step with the equity
    actually carried forward. (Every waterfall/sum check passes in that case
    because the bad figure is self-consistent within the income statement.)"""
    issues: List[str] = []
    bs = get_balance_sheet(data)
    pnl = get_income_statement(data)
    if not isinstance(bs, dict) or not isinstance(pnl, dict) or not bs or not pnl:
        return issues
    equity = bs.get("equity", {})
    if not isinstance(equity, dict):
        return issues
    re_vals = _node_values(equity.get("retained_earnings", {}))
    eq_vals = _node_values(equity)
    sc_vals = _node_values(equity.get("share_capital", {}))
    npat = _node_values(pnl.get("net_profit_after_tax", {}))
    div = pnl.get("dividends", {})
    div_vals = _node_values(div.get("declared", {})) if isinstance(div, dict) else {}

    pks = get_period_keys(data)
    for prev, cur in zip(pks, pks[1:]):
        # Only enforce between comparable audited periods; management-account or
        # YTD periods carry equity on a different basis and would false-positive.
        if get_period_type(data, prev) != "audited" or get_period_type(data, cur) != "audited":
            continue
        np_c = npat.get(cur)
        if not isinstance(np_c, (int, float)):
            continue
        d_c = div_vals.get(cur)
        d_c = d_c if isinstance(d_c, (int, float)) else 0
        # Prefer retained earnings (excludes share-capital movements); fall back
        # to total equity net of the share-capital change.
        re_p, re_c = re_vals.get(prev), re_vals.get(cur)
        if isinstance(re_p, (int, float)) and isinstance(re_c, (int, float)):
            actual, basis = re_c - re_p, "retained earnings"
        else:
            te_p, te_c = eq_vals.get(prev), eq_vals.get(cur)
            if not (isinstance(te_p, (int, float)) and isinstance(te_c, (int, float))):
                continue
            sc_p = sc_vals.get(prev) if isinstance(sc_vals.get(prev), (int, float)) else 0
            sc_c = sc_vals.get(cur) if isinstance(sc_vals.get(cur), (int, float)) else 0
            actual = (te_c - te_p) - (sc_c - sc_p)
            basis = "total equity net of share-capital change"
        expected = np_c - d_c  # profit retained this period
        if abs(actual - expected) > SUM_TOLERANCE:
            issues.append(
                f"P&L-TO-BALANCE-SHEET BREAK for {cur}: net profit {_fmt(np_c)} less "
                f"dividends {_fmt(d_c)} = {_fmt(expected)} retained, but {basis} moved "
                f"{_fmt(actual)} from {prev} to {cur} (difference {_fmt(actual - expected)}). "
                f"These must reconcile. Re-verify the income statement — an expense or "
                f"sub-total may be mis-read so net profit is wrong even though the P&L foots "
                f"internally — and the carried-forward equity. If the gap is a genuine "
                f"prior-year restatement, other comprehensive income, or a reserve transfer, "
                f"surface that item explicitly rather than leaving the two statements at odds."
            )
    return issues

# Borrowings = debt facilities. Scanned over liability sections only, where a
# bare "loan" key is always a borrowing.
_BORROWING_KEY_RE = re.compile(
    r"borrow|\bloans?\b|hire[\s_]*purchase|finance[\s_]*lease|"
    r"overdraft|trade[\s_]*financ|invoice[\s_]*financ|factoring|bridging|sukuk|"
    r"debenture|lease[\s_]*liabilit", re.I)
_PAYABLE_KEY_RE = re.compile(r"payable", re.I)

def _borrowings_by_period(bs: Dict) -> Dict:
    out: Dict = {}
    for section_name in ("non_current_liabilities", "current_liabilities"):
        section = bs.get(section_name)
        if not isinstance(section, dict):
            continue
        for key, item in _section_items(section).items():
            hay = f"{key} {item.get('display_name', '')}"
            # Borrowing-name match wins: "finance lease payables" / "hire
            # purchase payables" are borrowings despite the word "payables".
            if not _BORROWING_KEY_RE.search(hay):
                continue
            for pk, v in _node_values(item).items():
                if isinstance(v, (int, float)):
                    out[pk] = out.get(pk, 0.0) + v
    return out

def check_ratio_recomputation(data: Dict) -> List[str]:
    """Recompute every stored ratio from the statements. LLMs approximate
    arithmetic, and a ratio computed from the wrong inputs (e.g. gearing that
    silently excludes current borrowings) survives every other check because
    it is internally consistent."""
    issues: List[str] = []
    bs = get_balance_sheet(data)
    pnl = get_income_statement(data)
    fr = data.get("financial_ratios", {})
    if not isinstance(bs, dict) or not isinstance(pnl, dict) or not isinstance(fr, dict) or not fr:
        return issues

    ca_section = bs.get("current_assets", {}) if isinstance(bs.get("current_assets"), dict) else {}
    cash = {}
    inventory = _node_values(_get_item(ca_section, "inventory"))
    for key, item in _section_items(ca_section).items():
        if "cash" in f"{key} {item.get('display_name', '')}".lower():
            for pk, v in _node_values(item).items():
                if isinstance(v, (int, float)):
                    cash[pk] = cash.get(pk, 0.0) + v
    inputs = {
        "ca": _node_values(bs.get("current_assets", {})),
        "cl": _node_values(bs.get("current_liabilities", {})),
        "ta": _node_values(bs.get("total_assets", {})),
        "tl": _node_values(bs.get("total_liabilities", {})),
        "te": _node_values(bs.get("equity", {})),
        "cash": cash,
        "inv": inventory,
        "borrowings": _borrowings_by_period(bs),
        "rev": _node_values(pnl.get("revenue", {})),
        "gp": _node_values(pnl.get("gross_profit", {})),
        "op": _node_values(pnl.get("operating_profit", {})),
        "pbt": _node_values(pnl.get("profit_before_tax", {})),
    ...
    def g(name, pk):
        v = inputs[name].get(pk)
        return v if isinstance(v, (int, float)) else None

    def ratio(num, den):
        return None if (num is None or den is None or den == 0) else num / den

    # (group, key(s), recompute fn, tolerance, human formula)
    specs = [
        ("liquidity_ratios", ("current_ratio",),
         lambda pk: ratio(g("ca", pk), g("cl", pk)), RATIO_TOLERANCE,
         "Current Assets / Current Liabilities"),
        ("liquidity_ratios", ("quick_ratio",),
         lambda pk: ratio(None if g("ca", pk) is None else g("ca", pk) - (g("inv", pk) or 0), g("cl", pk)),
         RATIO_TOLERANCE, "(Current Assets - Inventory) / Current Liabilities"),
        ("liquidity_ratios", ("cash_ratio",),
         lambda pk: ratio(g("cash", pk), g("cl", pk)), RATIO_TOLERANCE,
         "Cash and Bank / Current Liabilities"),
        ("leverage_ratios", ("liabilities_to_equity", "debt_to_equity"),
         lambda pk: ratio(g("tl", pk), g("te", pk)), RATIO_TOLERANCE,
         "Total Liabilities / Total Equity"),
        ("leverage_ratios", ("liabilities_to_assets", "debt_to_assets"),
         lambda pk: ratio(g("tl", pk), g("ta", pk)), RATIO_TOLERANCE,
         "Total Liabilities / Total Assets"),
        ("leverage_ratios", ("gearing_ratio",),
         lambda pk: ratio(g("borrowings", pk), g("te", pk)), RATIO_TOLERANCE,
         "Total Borrowings (ALL interest-bearing debt, current + non-current) / Total Equity"),
        ("leverage_ratios", ("interest_coverage",),
         lambda pk: ratio(g("ebitda", pk), g("fc", pk)), RATIO_TOLERANCE,
         "EBITDA / Finance Costs"),
        ("profitability_ratios", ("gross_profit_margin",),
         lambda pk: None if ratio(g("gp", pk), g("rev", pk)) is None else ratio(g("gp", pk), g("rev", pk)) * 100,
         PCT_TOLERANCE, "(Gross Profit / Revenue) x 100"),
        ("profitability_ratios", ("operating_profit_margin",),
         lambda pk: None if ratio(g("op", pk), g("rev", pk)) is None else ratio(g("op", pk), g("rev", pk)) * 100,
         PCT_TOLERANCE, "(Operating Profit / Revenue) x 100"),
        ("profitability_ratios", ("pbt_margin",),
         lambda pk: None if ratio(g("pbt", pk), g("rev", pk)) is None else ratio(g("pbt", pk), g("rev", pk)) * 100,
         PCT_TOLERANCE, "(PBT / Revenue) x 100"),
        ("profitability_ratios", ("net_profit_margin",),
         lambda pk: None if ratio(g("npat", pk), g("rev", pk)) is None else ratio(g("npat", pk), g("rev", pk)) * 100,
         PCT_TOLERANCE, "(NPAT / Revenue) x 100"),
        ("profitability_ratios", ("ebitda_margin",),
         lambda pk: None if ratio(g("ebitda", pk), g("rev", pk)) is None else ratio(g("ebitda", pk), g("rev", pk)) * 100,
         PCT_TOLERANCE, "(EBITDA / Revenue) x 100"),
        ("profitability_ratios", ("roa",),
         lambda pk: None if ratio(g("npat", pk), g("ta", pk)) is None else ratio(g("npat", pk), g("ta", pk)) * 100,
         PCT_TOLERANCE, "(NPAT / Total Assets) x 100"),
        ("profitability_ratios", ("roe",),
         lambda pk: None if ratio(g("npat", pk), g("te", pk)) is None else ratio(g("npat", pk), g("te", pk)) * 100,
         PCT_TOLERANCE, "(NPAT / Total Equity) x 100"),
    ]
    for group, keys, fn, tol, formula in specs:
        grp = fr.get(group)
        ...
        used_key = keys[0]
        for k in keys:
            if isinstance(grp.get(k), dict):
                node, used_key = grp[k], k
                break
        if node is None:
            continue
        stored_vals = node.get("values", {})
        if not isinstance(stored_vals, dict):
            continue
        for pk, stored in stored_vals.items():
            if not isinstance(stored, (int, float)):
                continue
            computed = fn(pk)
            if computed is None:
                continue
            if abs(stored - round(computed, 2)) > tol:
                issues.append(
                    f"RATIO MISMATCH: financial_ratios.{group}.{used_key}.values.{pk} "
                    f"= {stored} but {formula} recomputes to {round(computed, 2)} from the "
                    f"statements in this same JSON. Recompute the ratio from the statement "
                    f"figures and overwrite the stored value. If the statements themselves "
                    f"are wrong, fix the statements first, then every ratio derived from them."
                )
    # Efficiency days: check both the standard (365-day) and period-adjusted variants.
    eff = fr.get("efficiency_ratios", {})
    if isinstance(eff, dict):
        tr = _node_values(_get_item(ca_section, "trade_receivables"))
        cl_section = bs.get("current_liabilities", {}) if isinstance(bs.get("current_liabilities"), dict) else {}
        tp = _node_values(_get_item(cl_section, "trade_payables"))
        cos = _node_values(pnl.get("cost_of_sales", {}))
        rev = inputs["rev"]
        inv = inputs["inv"]

        def days_fn(numer, denom, pk, day_count):
            n, d = numer.get(pk), denom.get(pk)
            if not isinstance(n, (int, float)) or not isinstance(d, (int, float)) or d == 0:
                return None
            return n / d * day_count

        for variant, default_days in (("values_standard", 365), ("values_period_adjusted", None)):
            for rk, numer, denom in (("debtor_days", tr, rev),
                                     ("creditor_days", tp, cos),
                                     ("inventory_days", inv, cos)):
                node = eff.get(rk)
                if not isinstance(node, dict):
                    continue
                stored_vals = node.get(variant, {})
                period_days = node.get("period_days", {})
                if not isinstance(stored_vals, dict):
                    continue
                for pk, stored in stored_vals.items():
                    if not isinstance(stored, (int, float)):
                    ...
                    if day_count is None:
                        pd_v = period_days.get(pk) if isinstance(period_days, dict) else None
                        if not isinstance(pd_v, (int, float)) or pd_v <= 0:
                            continue
                        day_count = pd_v
                    computed = days_fn(numer, denom, pk, day_count)
                    if computed is None:
                        continue
                    if abs(stored - computed) > DAYS_TOLERANCE:
                        issues.append(
                            f"RATIO MISMATCH: financial_ratios.efficiency_ratios.{rk}."
                            f"{variant}.{pk} = {stored} days but the statements recompute "
                            f"to {computed:.1f} days using a {day_count}-day basis. "
                            f"Recompute from the statement figures and overwrite the stored "
                            f"value (keep values_standard on 365 days and "
                            f"values_period_adjusted on the period's actual day count)."
                        )
    return issues

def check_cross_section_figures(data: Dict) -> List[str]:
    """Figures restated across sections (working capital, TNW, funding
    mismatch, DSCR, funding profile) must match the statements exactly.
    Two production reports for the same company once disagreed on trade
    receivables, TNW adjustments, and the funding-profile borrowings total —
    every one of those drifts is caught here."""
    issues: List[str] = []
    bs = get_balance_sheet(data)
    pnl = get_income_statement(data)
    if not isinstance(bs, dict) or not bs:
    ...
    cl_section = bs.get("current_liabilities", {}) if isinstance(bs.get("current_liabilities"), dict) else {}
    tr = _node_values(_get_item(ca_section, "trade_receivables"))
    inv = _node_values(_get_item(ca_section, "inventory"))
    tp = _node_values(_get_item(cl_section, "trade_payables"))
    eq = _node_values(bs.get("equity", {}))
    nca = _node_values(bs.get("non_current_assets", {}))
    ncl = _node_values(bs.get("non_current_liabilities", {}))

    def _diff(pk, path, stated, ref, ref_name):
        if isinstance(stated, (int, float)) and isinstance(ref, (int, float)) \
                and abs(stated - ref) > SUM_TOLERANCE:
            issues.append(
                f"CROSS-SECTION DRIFT for {pk}: {path} = {_fmt(stated)} but {ref_name} "
                f"= {_fmt(ref)} (difference {_fmt(stated - ref)}). The same figure must "
                f"be identical everywhere it appears. Overwrite the drifted copy with "
                f"the balance-sheet/P&L value."
            )

    # Working capital components vs balance sheet
    wc = data.get("working_capital_analysis", {})
    owc = wc.get("operating_working_capital", {}) if isinstance(wc, dict) else {}
    components = owc.get("components", {}) if isinstance(owc, dict) else {}
    owc_vals = owc.get("values", {}) if isinstance(owc, dict) else {}
    if isinstance(components, dict):
        for pk, comp in components.items():
            if not isinstance(comp, dict):
                continue
            _diff(pk, f"working_capital_analysis.operating_working_capital.components.{pk}.trade_receivables",
                  comp.get("trade_receivables"), tr.get(pk),
                  "statement_of_financial_position.current_assets.trade_receivables")
            _diff(pk, f"working_capital_analysis.operating_working_capital.components.{pk}.inventory",
                  comp.get("inventory"), inv.get(pk),
                  "statement_of_financial_position.current_assets.inventory")
            _diff(pk, f"working_capital_analysis.operating_working_capital.components.{pk}.trade_payables",
                  comp.get("trade_payables"), tp.get(pk),
                  "statement_of_financial_position.current_liabilities.trade_payables")
            stated_owc = owc_vals.get(pk) if isinstance(owc_vals, dict) else None
            parts = (comp.get("trade_receivables"), comp.get("inventory"), comp.get("trade_payables"))
            if isinstance(stated_owc, (int, float)) and all(isinstance(p, (int, float)) for p in parts):
                computed = parts[0] + parts[1] - parts[2]
                if abs(stated_owc - computed) > SUM_TOLERANCE:
                    issues.append(
                        f"FORMULA MISMATCH for {pk}: working_capital_analysis."
                        f"operating_working_capital.values.{pk} = {_fmt(stated_owc)} but its own "
                        f"components give {_fmt(parts[0])} + {_fmt(parts[1])} - {_fmt(parts[2])} "
                        f"= {_fmt(computed)}. Recompute OWC from the components."
                    )
    # TNW vs equity
    tnw = data.get("tnw_analysis", {})
    calc = tnw.get("calculation", {}) if isinstance(tnw, dict) else {}
    if isinstance(calc, dict):
        for pk, blk in calc.items():
            if not isinstance(blk, dict):
                continue
            _diff(pk, f"tnw_analysis.calculation.{pk}.original_tnw",
                  blk.get("original_tnw"), eq.get(pk),
                  "statement_of_financial_position.equity.total")
            adj = blk.get("adjustments", {})
            if isinstance(adj, dict):
                stated_total = adj.get("total_adjustments")
                parts_sum = sum(v for k, v in adj.items()
                                if k != "total_adjustments" and isinstance(v, (int, float)))
                if isinstance(stated_total, (int, float)) and abs(stated_total - parts_sum) > SUM_TOLERANCE:
                    issues.append(
                        f"SUM MISMATCH for {pk}: tnw_analysis.calculation.{pk}.adjustments."
                        f"total_adjustments = {_fmt(stated_total)} but the individual "
                        f"adjustments sum to {_fmt(parts_sum)}. Make them agree."
                    )
                a, o = blk.get("adjusted_tnw"), blk.get("original_tnw")
                if all(isinstance(x, (int, float)) for x in (a, o, stated_total)) \
                        and abs(a - (o - stated_total)) > SUM_TOLERANCE:
                    issues.append(
                        f"FORMULA MISMATCH for {pk}: tnw_analysis.calculation.{pk}."
                        f"adjusted_tnw = {_fmt(a)} but original_tnw - total_adjustments = "
                        f"{_fmt(o)} - {_fmt(stated_total)} = {_fmt(o - stated_total)}. Recompute."
                    )
    # Funding mismatch vs balance sheet
    fm = data.get("funding_mismatch_analysis", {})
    layer1 = fm.get("layer_1_gap_identification", {}) if isinstance(fm, dict) else {}
    if isinstance(layer1, dict):
        for pk, blk in layer1.items():
            if not isinstance(blk, dict):
                continue
            _diff(pk, f"funding_mismatch_analysis.layer_1_gap_identification.{pk}.non_current_assets",
                  blk.get("non_current_assets"), nca.get(pk),
                  "statement_of_financial_position.non_current_assets.total")
            ltf = blk.get("long_term_funding", {})
            if isinstance(ltf, dict):
                _diff(pk, f"funding_mismatch_analysis.layer_1_gap_identification.{pk}.long_term_funding.total_equity",
                      ltf.get("total_equity"), eq.get(pk),
                      "statement_of_financial_position.equity.total")
                _diff(pk, f"funding_mismatch_analysis.layer_1_gap_identification.{pk}.long_term_funding.non_current_liabilities",
                      ltf.get("non_current_liabilities"), ncl.get(pk),
                      "statement_of_financial_position.non_current_liabilities.total")
                t, e, n = ltf.get("total"), ltf.get("total_equity"), ltf.get("non_current_liabilities")
                if all(isinstance(x, (int, float)) for x in (t, e, n)) and abs(t - (e + n)) > SUM_TOLERANCE:
                    issues.append(
                        f"SUM MISMATCH for {pk}: long_term_funding.total = {_fmt(t)} but "
                        f"total_equity + non_current_liabilities = {_fmt(e)} + {_fmt(n)} "
                        f"= {_fmt(e + n)}. Make them agree."
                    )
                gap, nca_v = blk.get("funding_gap"), blk.get("non_current_assets")
                if all(isinstance(x, (int, float)) for x in (gap, nca_v, t)) \
                        and abs(gap - (nca_v - t)) > SUM_TOLERANCE:
                    issues.append(
                        f"FORMULA MISMATCH for {pk}: funding_gap = {_fmt(gap)} but "
                        f"non_current_assets - long_term_funding.total = {_fmt(nca_v)} - "
                        f"{_fmt(t)} = {_fmt(nca_v - t)}. Recompute."
                    )
    # DSCR EBITDA vs P&L EBITDA
    ebitda = _node_values(pnl.get("ebitda", {})) if isinstance(pnl, dict) else {}
    dscr_calc = data.get("dscr_analysis", {}).get("calculation", {}) \
        if isinstance(data.get("dscr_analysis"), dict) else {}
    if isinstance(dscr_calc, dict):
        for pk, blk in dscr_calc.items():
            if isinstance(blk, dict):
                _diff(pk, f"dscr_analysis.calculation.{pk}.ebitda",
                      blk.get("ebitda"), ebitda.get(pk),
                      "statement_of_comprehensive_income.ebitda")
    # Funding profile internal sums
    fp = data.get("funding_profile", {})
    facilities = fp.get("existing_facilities_identified", {}) if isinstance(fp, dict) else {}
    if isinstance(facilities, dict) and facilities:
        facility_total_sum, saw_facility = 0.0, False
        for fk, blk in facilities.items():
            if fk == "total_borrowings" or not isinstance(blk, dict):
                continue
            cur, nc, tot = blk.get("current_portion"), blk.get("non_current_portion"), blk.get("total")
            if all(isinstance(x, (int, float)) for x in (cur, nc, tot)) \
                    and abs(tot - (cur + nc)) > SUM_TOLERANCE:
                issues.append(
                    f"SUM MISMATCH: funding_profile.existing_facilities_identified.{fk}."
                    f"total = {_fmt(tot)} but current_portion + non_current_portion = "
                    f"{_fmt(cur)} + {_fmt(nc)} = {_fmt(cur + nc)}. Make them agree."
                )
            amount = tot if isinstance(tot, (int, float)) else blk.get("amount")
            if isinstance(amount, (int, float)):
                facility_total_sum += amount
                saw_facility = True
        stated_total = facilities.get("total_borrowings")
        if saw_facility and isinstance(stated_total, (int, float)) \
                and abs(stated_total - facility_total_sum) > SUM_TOLERANCE:
            issues.append(
                f"SUM MISMATCH: funding_profile.existing_facilities_identified."
                f"total_borrowings = {_fmt(stated_total)} but the listed facilities sum to "
                f"{_fmt(facility_total_sum)} (difference {_fmt(stated_total - facility_total_sum)}). "
                f"total_borrowings must equal the sum of every facility listed (count each "
                f"facility once — never add a facility's current portion on top of a total "
                f"that already includes it)."
            )
    return issues

_PERIOD_SHAPE_RE = re.compile(r"[^a-z0-9]+")

def _canon_pk(key: str) -> str:
    return _PERIOD_SHAPE_RE.sub("", str(key).lower())

def check_line_items_shape(data: Dict) -> List[str]:
    """line_items must be ITEM-NAME-FIRST: {"item_name": {"display_name": ...,
    "values": {period: amount}}}. The schema leaves the inner shape open, so
    runs occasionally invert it to PERIOD-FIRST ({"fy2023": {"item": amount}})
    — the renderer then silently drops the whole breakdown. Hard-fail the
    wrong nesting so it never reaches a delivered report."""
    issues: List[str] = []
    period_canon = {_canon_pk(pk) for pk in get_period_keys(data)}
    if not period_canon:
        return issues

    def _walk(path: str, container) -> None:
        line_items = container.get("line_items") if isinstance(container, dict) else None
        if not isinstance(line_items, dict):
            return
        period_keys_found = [k for k in line_items if _canon_pk(k) in period_canon]
        if period_keys_found:
            issues.append(
                f"WRONG line_items SHAPE at {path}.line_items: keys {period_keys_found} "
                f"are PERIOD keys — the structure is period-first. line_items must be "
                f"ITEM-NAME-FIRST: each key is an item name mapping to "
                f'{{"display_name": "...", "values": {{"<period>": <amount>, ...}}}}. '
                f"Rebuild this section in the correct shape (the renderer cannot display "
                f"period-first nesting)."
            )
            return
        for k, item in line_items.items():
            if not isinstance(item, dict) or not isinstance(item.get("values"), dict):
                issues.append(
                    f"MALFORMED line item at {path}.line_items.{k}: every line item must "
                    f'be an object with a "values" dict keyed by period '
                    f'({{"values": {{"<period>": <amount>}}}}).'
                )

    pnl = get_income_statement(data)
    if isinstance(pnl, dict):
        for key in ("revenue", "cost_of_sales", "other_income", "finance_costs", "taxation"):
            _walk(f"statement_of_comprehensive_income.{key}", pnl.get(key))
        opex = pnl.get("operating_expenses")
        if isinstance(opex, dict):
            for sub in ("administrative_expenses", "other_expenses"):
                _walk(f"statement_of_comprehensive_income.operating_expenses.{sub}", opex.get(sub))
    bs = get_balance_sheet(data)
    if isinstance(bs, dict):
        for key in ("non_current_assets", "current_assets", "equity",
                    "non_current_liabilities", "current_liabilities"):
            section = bs.get(key)
            _walk(f"statement_of_financial_position.{key}", section)
            if isinstance(section, dict):
                for child_key, child in section.items():
                    if child_key not in ("total", "line_items") and isinstance(child, dict):
                        _walk(f"statement_of_financial_position.{key}.{child_key}", child)
    return issues

_BANK_CHARGE_RE = re.compile(r"bank[\s_]*charge", re.I)
_INTEREST_RE = re.compile(r"interest|finance[\s_]*cost|loan[\s_]*charge", re.I)

def check_classification_warnings(data: Dict) -> List[str]:
    """Soft conventions (warnings, not errors): finance costs carry interest
    only; bank charges belong in administrative expenses. Two reports for the
    same company once classified the same RM41k bank charges differently,
    shifting EBIT/EBITDA/interest-coverage between them."""
    warnings: List[str] = []
    pnl = get_income_statement(data)
    if not isinstance(pnl, dict):
        return warnings
    fc = pnl.get("finance_costs", {})
    fc_items = fc.get("line_items", {}) if isinstance(fc, dict) else {}
    if isinstance(fc_items, dict):
        for key, item in fc_items.items():
            hay = f"{key} {item.get('display_name', '') if isinstance(item, dict) else ''}"
            if _BANK_CHARGE_RE.search(hay):
                warnings.append(
                    f"CLASSIFICATION: finance_costs.line_items.{key} looks like bank "
                    f"charges. Convention: bank charges sit in administrative expenses; "
                    f"finance costs carry interest only (this keeps DSCR/interest-coverage "
                    f"denominators consistent across reports)."
                )
    opex = pnl.get("operating_expenses", {})
    adm = opex.get("administrative_expenses", {}) if isinstance(opex, dict) else {}
    adm_items = adm.get("line_items", {}) if isinstance(adm, dict) else {}
    if isinstance(adm_items, dict):
        for key, item in adm_items.items():
            hay = f"{key} {item.get('display_name', '') if isinstance(item, dict) else ''}"
            if _INTEREST_RE.search(hay) and not _BANK_CHARGE_RE.search(hay):
                vals = _node_values(item)
                if any(isinstance(v, (int, float)) and v != 0 for v in vals.values()):
                    warnings.append(
                        f"CLASSIFICATION: operating_expenses.administrative_expenses."
                        f"line_items.{key} looks like an interest/finance charge. "
                        f"Convention: interest belongs in finance_costs (it must reach the "
                        f"DSCR and interest-coverage denominators)."
                    )
    return warnings

_COMBINED_PAYABLES_RE = re.compile(r"trade[\s_]*(and|&)[\s_]*other[\s_]*payable", re.I)

def check_payables_granularity_warnings(data: Dict) -> List[str]:
    """Tripwires for the combined-payables failure mode (framework v7.9.4):
    using the 'trade and other payables' face line as trade payables inflates
    creditor days to implausible levels (a live MTC run produced 419 days) and
    overstates the negative CCC/WCR. The audited payables note usually
    provides the trade/other split — re-extraction, not tolerance, is the fix."""
    warnings: List[str] = []
    bs = get_balance_sheet(data)
    if isinstance(bs, dict):
        for section_name in ("current_liabilities", "non_current_liabilities"):
            section = bs.get(section_name)
            if not isinstance(section, dict):
                continue
            for key, item in _section_items(section).items():
                hay = f"{key} {item.get('display_name', '')}"
                if _COMBINED_PAYABLES_RE.search(hay):
                    warnings.append(
                        f"GRANULARITY: statement_of_financial_position.{section_name}.{key} "
                        f"is a combined trade-and-other-payables line. If the payables note "
                        f"or an attached schedule splits trade creditors from other payables/"
                        f"accruals, extract them separately — creditor days and OWC must use "
                        f"TRADE payables only (framework v7.9.4 rule)."
                    )
    eff = data.get("financial_ratios", {})
    eff = eff.get("efficiency_ratios", {}) if isinstance(eff, dict) else {}
    cd = eff.get("creditor_days", {}) if isinstance(eff, dict) else {}
    if isinstance(cd, dict):
        for variant in ("values_standard", "values"):
            vals = cd.get(variant)
            if not isinstance(vals, dict):
                continue
            for pk, v in vals.items():
                if isinstance(v, (int, float)) and v > 365:
                    warnings.append(
                        f"PLAUSIBILITY: creditor_days.{variant}.{pk} = {v} days exceeds a "
                        f"full year — almost always a sign that a combined trade-and-other-"
                        f"payables figure was used as trade payables. Re-extract the trade/"
                        f"other split from the payables note and recompute creditor days, "
                        f"CCC and WCR."
                    )
            break  # variants carry the same numbers — check one to avoid duplicates
    return warnings

def run_financial_consistency_checks(data: Dict) -> Tuple[List[str], List[str]]:
    """Aggregate the consistency suite. Returns (errors, warnings). Each check
    is fenced so a malformed JSON degrades to a warning instead of breaking
    the render path."""
    errors: List[str] = []
    warnings: List[str] = []
    for fn in (check_line_items_shape, check_statement_completeness,
               check_section_sums, check_balance_sheet_identities,
               check_pnl_chain, check_equity_articulation,
               check_ratio_recomputation,
               check_cross_section_figures):
        try:
            errors.extend(fn(data))
        except Exception as e:
            warnings.append(f"{fn.__name__} could not run on this JSON ({e!r}) — check skipped")
    for warn_fn in (check_classification_warnings, check_payables_granularity_warnings):
        try:
            warnings.extend(warn_fn(data))
        except Exception as e:
            warnings.append(f"{warn_fn.__name__} could not run ({e!r})")
...
body{margin:0;padding:32px 16px 40px;font-family:Inter,system-ui,-apple-system,sans-serif;background:var(--body-bg);color:var(--text-main);line-height:1.6;transition:background 0.3s ease, color 0.3s ease}
...
def generate_header(data: Dict) -> str:
    ...
    # Always stamp the actual render time. The model cannot know today's date and
    # routinely emits a stale generation_date (e.g. 2025-01-15), so never trust it.
    generated_date = datetime.now().strftime("%Y-%m-%d")
    generated_by = schema_info.get("generated_by") or "Kredit Lab"
    
    # Build auditor info for header if available. Show the FIRM (auditor of
    # record), not the individual partner; fall back for older reports.
    auditor_info = ""
    for pk in period_keys:
        opinion_data = audit_opinion.get(pk, {})
        if isinstance(opinion_data, dict):
            firm = (
                opinion_data.get("audit_firm")
                or opinion_data.get("audit_firm_number")
                or opinion_data.get("auditor_name")
                or ""
            ).strip()
            if firm:
                auditor_info = f"<div><div class='meta-label'>Auditor</div><div class='meta-value'>{firm}</div></div>"
...
def generate_funding_profile_section(data: Dict) -> str:
    ...
    if facilities:
        ...
        rows_total = 0  # accumulate rendered facility rows so the grand total foots to them
        ...
        for key in ["hire_purchase", "term_loan", "overdraft", "trade_financing",
            ...
            if item:
                ...
                # A facility row's Total must equal its own Current + Non-Current.
                # Only trust a supplied 'total' when no split is provided, so a
                # model that writes the portfolio total here can't distort the row.
                if "current_portion" in item or "non_current_portion" in item:
        ...
        # Grand total must foot to the rows shown above. Derive it from the
        # rendered rows; fall back to the model's figure only if no rows exist.
        total_borrowings = rows_total if rows_total != 0 else facilities.get("total_borrowings", 0)
...
def generate_integrity_section(data: Dict) -> str:
    """Independent integrity panel. Recomputes the balance-sheet identity from the
    statement's OWN section totals (NCA, CA, equity, NCL, CL) rather than echoing
    the model's self-reported integrity_check — which can claim 'balanced' while
    the figures do not foot. The status shown here is verified by the engine."""
    period_keys = get_period_keys(data)
    bs = get_balance_sheet(data)
    if not isinstance(bs, dict) or not bs:
        return ""
    ...
    # Recompute both sides per period, independent of the model's stated totals.
    assets = {pk: _sum([nca.get(pk), ca.get(pk)]) for pk in period_keys}
    eqliab = {pk: _sum([eq.get(pk), ncl.get(pk), cl.get(pk)]) for pk in period_keys}

    header_cells = "<th>Check (independently verified)</th>"
    for pk in period_keys:
        header_cells += f'<th class="number">{get_period_label(data, pk)}</th>'

    body = ""

    # Total Assets (recomputed)
    cells = '<td class="indent-1">Total Assets (NCA + CA)</td>'
    for pk in period_keys:
        v = assets.get(pk)
        cells += f'<td class="number">{format_number_or_dash(v)}</td>'
    body += f'<tr>{cells}</tr>'

    # Total Equity & Liabilities (recomputed)
    cells = '<td class="indent-1">Total Equity &amp; Liabilities (Equity + NCL + CL)</td>'
    for pk in period_keys:
        v = eqliab.get(pk)
        cells += f'<td class="number">{format_number_or_dash(v)}</td>'
    body += f'<tr>{cells}</tr>'
    ...
    for pk in period_keys:
        a, b = assets.get(pk), eqliab.get(pk)
        if isinstance(a, (int, float)) and isinstance(b, (int, float)):
            var = a - b
            balanced = abs(var) <= SUM_TOLERANCE
            if not balanced:
                any_imbalance = True
            status_class = "positive" if balanced else "negative"
            cells += f'<td class="number {status_class}"><strong>{format_number(var)}</strong></td>'
        else:
            cells += '<td class="number">&mdash;</td>'
    body += f'<tr class="total-row">{cells}</tr>'
    ...
    for pk in period_keys:
        a, b = assets.get(pk), eqliab.get(pk)
        if isinstance(a, (int, float)) and isinstance(b, (int, float)):
            balanced = abs(a - b) <= SUM_TOLERANCE
            status = "✅ Balanced" if balanced else "❌ Does NOT balance"
            status_class = "positive" if balanced else "negative"
    ...
    if any_imbalance:
        note = (
            '<div class="note-box danger"><strong>⚠️ Independent verification:</strong> '
            'the balance sheet does NOT foot for one or more periods &mdash; Total Assets '
            'does not equal Equity + Liabilities when recomputed from the statement’s own '
            'section totals. This check is performed by the Kredit Lab engine and overrides '
            'any &ldquo;balanced&rdquo; self-assessment in the source data. Treat the figures '
            'as provisional and re-run or correct the extraction before relying on them.</div>'
        )

    return f'''<div class="collapsible-section" id="section-integrity"><button class="section-toggle active" onclick="toggleSection(this)"><div class="toggle-left"><div class="toggle-icon-wrapper">✅</div><div class="toggle-text"><h3>Integrity Check</h3><p>Balance sheet verification (independently recomputed)</p></div></div><div class="toggle-arrow">▼</div></button><div class="section-content"><div class="table-card"><div class="table-wrapper"><table><thead><tr>{header_cells}</tr></thead><tbody>{body}</tbody></table></div></div>{note}</div></div>'''
...
def generate_ma_limitations_section(data: Dict) -> str:
    """Render the _ma_limitations top-level section (v7.7+ schema).
    Surfaces MA-specific accounting quirks the analyst must factor into
    interpretation: missing depreciation, improper equity carryforward,
    netted-out liability balances, etc. Hidden entirely if no MA periods.
    """
    ma = data.get("_ma_limitations") or {}
    if not ma:
        return ""

    blocks = ""
    for period_key, notes in ma.items():
        if not isinstance(notes, list) or not notes:
            continue
        # period_key like "fy2025_notes" -> label "FY2025"
        period = period_key.replace("_notes", "").upper()
        items = "".join(f"<li>{n}</li>" for n in notes)
        blocks += (
            f'<div class="note-box warning" style="margin-bottom:12px;">'
            f'<strong>{period} (Management Accounts):</strong>'
            f'<ul style="margin:8px 0 0 1.2em;padding:0;">{items}</ul>'
            f'</div>'
        )
    ...
    return (
        '<div class="collapsible-section" id="section-ma-limitations">'
        '<button class="section-toggle active" onclick="toggleSection(this)">'
        '<div class="toggle-left">'
        '<div class="toggle-icon-wrapper">⚠️</div>'
        '<div class="toggle-text">'
        '<h3>Management Accounts — Limitations</h3>'
        '<p>Known data-quality caveats analysts must factor into MA period interpretation</p>'
        '</div></div><div class="toggle-arrow">▼</div></button>'
        f'<div class="section-content">{blocks}</div></div>'
    )
...
def generate_audit_opinion_section(data: Dict) -> str:
    ...
    for pk in period_keys:
        ...
        audit_firm = (opinion_data.get("audit_firm") or "").strip()
        auditor_name = (opinion_data.get("auditor_name") or "").strip()  # individual partner
        audit_firm_number = (opinion_data.get("audit_firm_number") or "").strip()
        date_signed = opinion_data.get("date_signed", "N/A")
        # The auditor of record is the FIRM. Fall back for older reports that only
        # captured the individual or put the firm name in the number field.
        firm_display = audit_firm or audit_firm_number or auditor_name or "N/A"
        ...
        content_html += f'''<div class="audit-opinion-item {opinion_class}">
            ...
            <div class="detail"><strong>Auditor:</strong> {firm_display}</div>'''

        # Show the individual engagement partner only when it is a distinct person.
        if auditor_name and auditor_name != firm_display:
            content_html += f'<div class="detail"><strong>Engagement partner:</strong> {auditor_name}</div>'

        # Show the registration number separately only when the firm has its own field
        # (otherwise it is already part of firm_display via the fallback).
        if audit_firm and audit_firm_number:
            content_html += f'<div class="detail"><strong>Firm reg.:</strong> {audit_firm_number}</div>'
            ...
            if line_items:
                ...
                for li in line_items:
                    if isinstance(li, dict):
                        # v7.7+ shape: {line_item, previously_stated, adjustment, as_restated}
                        name = li.get("line_item", "")
                        prev = li.get("previously_stated")
                        adj = li.get("adjustment")
                        restated = li.get("as_restated")
                        parts = []
                        if prev is not None:
                            parts.append(f"was {format_number(prev)}")
                        if adj is not None:
                            parts.append(f"adj {format_number(adj)}")
                        if restated is not None:
                            parts.append(f"restated {format_number(restated)}")
                        detail = f" ({'; '.join(parts)})" if parts else ""
                        content_html += f'<li><strong>{name}</strong>{detail}</li>'
                    else:
...
def generate_footer(data: Dict) -> str:
    ...
    # Always the real render time — never the model's invented generation_date.
    generated_date = datetime.now().strftime("%Y-%m-%d")
...
{generate_ma_limitations_section(data)}
    ...
    if uploaded_file is not None:
        try:
            ...
            with tab1:
                ...
                st.markdown("---")
                st.subheader("Financial Consistency Check")
                st.caption("Recomputes every total, accounting identity, ratio and cross-section figure from the raw line items — independent of the report's own integrity_check.")
                fin_errors, fin_warnings = run_financial_consistency_checks(data)
                if fin_errors:
                    st.error(f"❌ {len(fin_errors)} consistency error(s) — figures below do NOT reconcile. Do not deliver this report without fixing them.")
                    for issue in fin_errors: st.error(issue)
                else:
                    st.success("✅ All figures reconcile (sums, identities, ratios, cross-sections)")
                if fin_warnings:
                    for w in fin_warnings: st.warning(f"⚠️ {w}")
            ...
            with tab4:
                ...
                try:
                    ...
                    excel_error_msg = str(e) if str(e) else type(e).__name__
                    excel_traceback = traceback.format_exc()
                    st.warning(f"⚠️ Excel generation encountered an issue: {excel_error_msg}")
                    with st.expander("🔍 Excel error details"):
                        st.code(excel_traceback, language="text")
                ...
                with col3:
                    if excel_ready and excel_bytes:
                        ...
                        if excel_error_msg:
                            st.caption(f"⚠️ Excel error: {excel_error_msg}")
                        else:
