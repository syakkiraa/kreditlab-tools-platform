# KREDIT LAB — BANK STATEMENT ANALYSIS SYSTEM PROMPT v3.5.5

> **v3.5.5 changelog** — Sprint 4.5 Option A. Parser's `account_type_determination.locked_type` enum is now `CR / OD / UNDETERMINED` only — dropped `CASH_LINE`. Islamic Cashline-i / CAP-i / SAP-i is the same revolving-credit facility as conventional Overdraft, so it locks as OD; no separate product class. Display-label enum `accounts[].account_type` collapses back to `Current / Savings / OD`. Mapping: `locked_type == OD` → display `OD`; `locked_type == CR` → `Current` or `Savings` per header text; `locked_type == UNDETERMINED` → `Current` default. The Cash Line / Cash Line-i disambiguation step in v3.5.4 is removed. Schema file reference bumps `v6_3_4` → `v6_3_5`; rulebook reference bumps `v3_4` → `v3_5`.
...
### Step 2: Detect account type (v3.5.5 — trust parser first)
