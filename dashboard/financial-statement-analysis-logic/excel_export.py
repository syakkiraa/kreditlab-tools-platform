...
# ---------------------------------------------------------------------------
# Value sanitization
# The analyzed JSON is produced by an AI model, so any narrative/value field
# may contain control characters (-> openpyxl IllegalCharacterError) or be an
# unexpected nested object/list (-> openpyxl "Cannot convert ... to Excel").
# Either one would abort the WHOLE workbook, which is why some files exported
# and others did not. We make openpyxl tolerant by coercing every cell value
# before it is bound, so a single bad field can never kill the export.
# ---------------------------------------------------------------------------
...
def _scalar(value):
    ...
    if isinstance(value, dict):
        # Readable shapes (e.g. directors as {"name": ..., "shares": ...})
        for k in ('name', 'display_name', 'title', 'label'):
            if isinstance(value.get(k), str):
                return _ILLEGAL_CHARS.sub('', value[k])
        # Common shapes for a wrapped numeric value (e.g. restated/adjusted entries)
        for k in ('value', 'amount', 'total', 'standard', 'current'):
            if k in value and isinstance(value[k], (int, float, str)):
                return _scalar(value[k])
        return _ILLEGAL_CHARS.sub('', str(value))
    if isinstance(value, (list, tuple, set)):
        return _ILLEGAL_CHARS.sub('', ', '.join(str(x) for x in value))
    return _ILLEGAL_CHARS.sub('', str(value))

# Patch openpyxl's cell binder once so every write goes through _scalar().
if not getattr(_openpyxl_cell.Cell, '_kreditlab_safe_bind', False):
    _orig_bind_value = _openpyxl_cell.Cell._bind_value
    def _safe_bind_value(self, value):
        return _orig_bind_value(self, _scalar(value))
    _openpyxl_cell.Cell._bind_value = _safe_bind_value
    _openpyxl_cell.Cell._kreditlab_safe_bind = True
...
def _num(x):
    """Pull a numeric scalar out of a value that may be wrapped in an object."""
    ...
    if isinstance(x, dict):
        for k in ('value', 'amount', 'total', 'standard', 'current'):
            if isinstance(x.get(k), (int, float)):
                return x[k]
        return 0
    return x  # strings (e.g. "N/A") pass through; binder strips control chars

def _gv(obj, pks):
    if isinstance(obj, dict):
        v = obj.get('values', obj)
        if isinstance(v, dict):
            return [_num(v.get(pk, 0)) for pk in pks]
    return [0]*len(pks)
...
def _dscr(wb, data, pks, labels):
    ...
    _tcp = tf.get('current_portions', {})
    if _tcp: _row(ws, r, ["  Total Term Current Portion", _tcp.get('total', 0)]+[None]*(len(pks)-1), True, nfmt=NF); r += 1
    _row(ws, r, ["Revolving: "+rv.get('description','')]+[None]*len(pks)); r += 1
    for f in rv.get('facilities', []): _row(ws, r, ["  "+f]+[None]*len(pks), indent=1); r += 1
    _ramt = rv.get('amounts', {})
    if _ramt: _row(ws, r, ["  Total Revolving (excl. from principal)", _ramt.get('total', 0)]+[None]*(len(pks)-1), True, nfmt=NF); r += 1
